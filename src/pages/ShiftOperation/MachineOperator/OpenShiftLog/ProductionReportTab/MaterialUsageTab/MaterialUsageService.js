...
export default function MaterialUsageService({
  ...
  const { NcId, NcProgramId, setPartDetailsData, servicedata, setService } =
    useGlobalContext();
        ...
        .then((response) => {
          ...
          const updatedServiceData = Object.values(groupedByPartId).flatMap(
            (group) => {
              ...
              if (
                useNow > totalQtyIssued ||
                rpTopData[0]?.QtyUsed + useNow + rpTopData[0]?.QtyReturned >
                  totalQtyIssued
              ) {
                hasValidationError = true;
              }
              ...
              const updatedGroup = group.map((item) => {
                ...
                const returnNowForItem = Math.min(
                  availableQty,
                  remainingUseNow
                ); // How much to return for this part
